...
export const App: React.FC = () => {
  ...
  // Show error if Ghostscript is not installed (no files selected yet)
          ...
          <Text>
            ...
            {isBatchMode ? (
              <>
                ...
                <Text color="#999999"> ({formatBytes(totalSize)} total - PDFs)</Text>
              </>
            ) : primaryFile && (
              <>
                <Text color="cyan">{primaryFile.name}</Text>
                <Text color="#999999"> ({formatBytes(primaryFile.size)} - PDF)</Text>
              </>
            )}
          </Text>
        </>
