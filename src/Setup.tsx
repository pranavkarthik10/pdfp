...
import { checkGhostscriptInstalled, getGhostscriptCommand } from './utils.js';
...
// Get Ghostscript version if installed
function getGhostscriptVersion(): string | null {
  try {
    const gsCommand = getGhostscriptCommand();
    const output = execSync(`${gsCommand} --version`, { stdio: 'pipe' }).toString();
    return output.trim();
  } catch {
    return null;
  }
...
function getInstallMethods(): InstallMethod[] {
  ...
  if (platform === 'linux') {
    methods.push({
      id: 'apt',
      name: 'APT',
      command: 'sudo apt-get install -y ghostscript',
    ...
    methods.push({
      id: 'yum',
      name: 'YUM',
      command: 'sudo yum install -y ghostscript',
...
export const Setup: React.FC<SetupProps> = ({ forceSetup = false, onComplete }) => {
  ...
  const [gsVersion, setGsVersion] = useState<string | null>(null);
  const installProcessRef = useRef<ReturnType<typeof spawn> | null>(null);

  // Initial check for Ghostscript
  useEffect(() => {
    const checkTimer = setTimeout(() => {
      const isInstalled = checkGhostscriptInstalled();
      const version = getGhostscriptVersion();
      setGsVersion(version);
      ...
      if (code === 0) {
        setTimeout(() => {
          if (checkGhostscriptInstalled()) {
            ...
            setErrorMessage('Installation completed but Ghostscript is not available. Restart your terminal.');
  ...
  return (
    <Box flexDirection="column">
      ...
      {step === 'checking' && (
        <Box marginTop={1}>
          <Spinner label="Checking for Ghostscript..." />
        </Box>
      )}
      ...
      {step === 'already-installed' && (
        <Box flexDirection="column" marginTop={1}>
          <Text>
            ...
            <Text color="#22c55e" bold>Ghostscript</Text>
            <Text color="white"> is already installed</Text>
            {gsVersion && <Text color="#666666"> (v{gsVersion})</Text>}
          </Text>
          <Box marginTop={1}>
            <Text color="#999999">You're all set! Ready to compress PDF files.</Text>
          </Box>
          <Box marginTop={1}>
      ...
      {step === 'not-installed' && (
        <Box flexDirection="column" marginTop={1}>
          <Text>
            ...
            <Text color="#ef4444" bold>Ghostscript</Text>
            <Text color="white"> is not installed</Text>
          </Text>
          <Box marginTop={1}>
            <Text color="#999999">Ghostscript is required to compress PDF files. We'll help you install it.</Text>
          </Box>
          <Box marginTop={1}>
      ...
      {step === 'installing' && (
        <Box flexDirection="column" marginTop={1}>
          <Spinner label="Installing Ghostscript..." />
          ...
          <Box marginTop={1} flexDirection="column">
            {process.platform === 'darwin' && (
              <>
                ...
                <Text color="#999999">2. Install Ghostscript:</Text>
                <Text color="cyan">   brew install ghostscript</Text>
              </>
            )}
            {process.platform === 'linux' && (
              <>
                <Text color="#999999">Debian/Ubuntu:</Text>
                <Text color="cyan">   sudo apt-get update && sudo apt-get install ghostscript</Text>
                <Text color="#999999">CentOS/RHEL:</Text>
                <Text color="cyan">   sudo yum install ghostscript</Text>
              </>
            )}
            {process.platform === 'win32' && (
              <>
                <Text color="#999999">1. Download from: https://www.ghostscript.com/download/gsdnld.html</Text>
                <Text color="#999999">2. Run the installer and add to PATH</Text>
              </>
            )}
          </Box>
          <Box marginTop={1}>
