...
import { QualityLevel, FileInfo, CompressionResult, AdvancedSettings, BatchFileInfo, QUALITY_OPTIONS } from './types.js';
import {
  getFileInfo,
  getFilesFromFolder,
  isDirectory,
...
export const FileDropper: React.FC<FileDropperProps> = ({ onFilesSelected }) => {
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color="yellow" bold>📁 Drop a PDF file or folder</Text>
      <Box marginTop={1}>
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Box marginTop={1}>
            <Text color="green">
              ...
              <Text color="#999999"> · {formatBytes(totalSize)} total · PDF</Text>
            </Text>
          </Box>
        </Box>
      )}
...
export const QualitySelect: React.FC<QualitySelectProps> = ({ fileInfo, onSelect }) => {
  const [selectedIndex, setSelectedIndex] = useState(1); // Default to ebook

  useInput((_input, key) => {
    if (key.upArrow) {
      setSelectedIndex((prev) => (prev > 0 ? prev - 1 : QUALITY_OPTIONS.length - 1));
    } else if (key.downArrow) {
      setSelectedIndex((prev) => (prev < QUALITY_OPTIONS.length - 1 ? prev + 1 : 0));
    } else if (key.return) {
      onSelect(QUALITY_OPTIONS[selectedIndex].value);
    }
  });

  const getQualityOption = (option: typeof QUALITY_OPTIONS[0], index: number) => {
    const isSelected = index === selectedIndex;
    const estimatedSize = estimateCompressedSize(fileInfo.size, option.value);
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color="yellow">⚙️  Select PDF quality:</Text>
      <Box marginTop={1} />
      <Box flexDirection="column">
        {QUALITY_OPTIONS.map((option, index) => getQualityOption(option, index))}
      </Box>
      <Box marginTop={1}>
...
export const Summary: React.FC<SummaryProps> = ({ result }) => {
  ...
  const displayFileName = truncateText(fileName, maxContentWidth - 20); // Reserve space for "🎉  → XX% smaller"
...
export const AdvancedSettingsPrompt: React.FC<AdvancedSettingsPromptProps> = ({ onSelect }) => {
  const [selectedIndex, setSelectedIndex] = useState(1); // Default to No
  const options = [
    { label: 'Yes', value: true, description: 'Configure output folder, target size' },
...
type AdvancedStep = 'output-folder' | 'target-size';
...
export const AdvancedSettingsEditor: React.FC<AdvancedSettingsEditorProps> = ({
  ...
  const [, setTargetSize] = useState<number | null>(null);
    ...
    if (key.return) {
      ...
      if (!value) {
        ...
        if (isNaN(numValue) || numValue <= 0) {
          ...
          onComplete({
            outputFolder,
            targetSize: unitToBytes(numValue, sizeUnit),
            targetSizeUnit: sizeUnit,
          });
        }
      }
